...
const int mxN=100, bw=3;
int t, n, tba[mxN][mxN], qu[mxN], qh, qt, prv[mxN], d[mxN], d2[mxN];
			...
			for(int i=0; i<n; ++i) {
				auto f1=[&](const int &j) {
					...
					for(int k : tbv[j]) {
						...
						int tca=sts[b][si].ca;
						sts[b][si].ca=i;
						int d3=tv(sts[b][si], k, j, 1);
						if(d3<n) {
							pq.push({sts[b][si].no+d2[i]+d3, si, i, j, k});
							if(pq.size()>bw)
								pq.pop();
						}
						sts[b][si].ca=tca;
